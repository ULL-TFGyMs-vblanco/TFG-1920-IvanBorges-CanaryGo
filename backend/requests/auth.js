...
app.post('/autorizar', function (req, res) {
  ...
  let usuario_activo = UsuarioLogueado()
...
app.delete('/autorizar', function (req, res) {
  ...
  if (req.body.opcion == "Borrar") {
    ...
    console.log('El rollo', usuario_activo.email)

    //  Obtenemos id de la db
    firebaseDb.collection('usuarios').where('Correo', "==", usuario_activo.email).get()
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          documento = doc.id
        })
      }).catch(function (error) {
        console.error("Error encontrando usuario en db");
      })
      .then(function () {
        // Borramos de la db
        ...
        console.error("Error borrando db");
      });

    // Borramos usuario
    usuario_activo.delete().then(function () {
      // User deleted.
...
function ActualizarInfo (nombre, genero, fecha, usuario, correo) {
  ...
  if (usuario_activo != false) {
    ...
    firebaseDb.collection('usuarios').add({
      ...
      .catch(function (error) {
        console.error('Error añadiendo usuario a la db')
      })
...
function ActualizarInfoDb (nombre, fecha, genero, correo) {
  ...
  let usuarios = firebaseDb.collection("usuarios");
  let usuario_activo = UsuarioLogueado()
  let documento
  ...
  firebaseDb.collection('usuarios').where('Correo', "==", correo).get()
    ...
    .then(function () {
      usuarios.doc(String(documento)).update({
        ...
        .catch(function (error) {
          console.error('Error al actualizar info en db', documento)
        })
    }).catch(function (error) {
...
function ActualizarCorreo (correo) {
  ...
  // Subir informacion
  if (usuario_activo != false) {
    usuario_activo.updatePassword(correo)
      .then(function () {
        // Update successful.
...
function ActualizarContrasena (contrasena) {
  ...
  if (usuario_activo != false) {
    usuario_activo.updatePassword(contrasena).then(function () {
      // Update successful.
